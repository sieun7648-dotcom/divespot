...
const AVAILABILITY_CACHE_TTL_MS = Math.max(
  30_000,
  Number(process.env.AVAILABILITY_CACHE_SECONDS || 300) * 1000
);
...
function sendJson(res, statusCode, payload) {
  ...
  res.writeHead(statusCode, apiHeaders({
...
async function loadAllProviders(date) {
  ...
  const request = Promise.all([
    ...
    const payload = { ok: true, date, facilities, cached: false };
    availabilityCache.set(date, {
      expiresAt: Date.now() + AVAILABILITY_CACHE_TTL_MS,
      payload
    });
    return payload;
  }).finally(() => availabilityInFlight.delete(date));

  availabilityInFlight.set(date, request);
  return request;
}
...
async function handleAvailability(reqUrl, res) {
  ...
  const payload = await loadAllProviders(date);
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "OPTIONS" && reqUrl.pathname.startsWith("/api/")) {
      res.writeHead(204, apiHeaders({ "Content-Length": "0" }));
      res.end();
      return;
    }
