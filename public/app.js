(() => {
  ...
  async function fetchFacility(key, date, signal) {
    const response = await fetchWithTimeout(
      apiUrl(`/api/availability?date=${encodeURIComponent(date)}&provider=${encodeURIComponent(key)}`),
      {
        headers: { Accept: "application/json" },
        cache: "no-store",
        signal
  ...
  async function load() {
    ...
    setLoading(true);
    setConnectionPending("서버 연결 중");
    els.time.textContent = "조회 준비 중...";
    ...
    const errors = {};
    renderState(state);
    setNotice("예약 조회 서버에 연결하고 있습니다.", "loading");

    const startupTimers = [
      setTimeout(() => {
        if (sequence === loadSequence && !signal.aborted) {
          setNotice("무료 조회 서버를 시작하고 있어요. 첫 조회는 최대 1분 정도 걸릴 수 있습니다.", "loading");
          els.time.textContent = "서버 시작 중...";
        }
      }, 3500),
      setTimeout(() => {
        if (sequence === loadSequence && !signal.aborted) {
          setNotice("서버가 깨어나는 중입니다. 화면을 닫지 않아도 준비되면 자동으로 표시됩니다.", "loading");
        }
      }, 30000)
    ];
    ...
    try {
      ...
      const message = "예약 조회 서버에 연결하지 못했습니다. 잠시 후 전체 새로고침을 눌러주세요.";
    ...
    const tasks = FACILITY_KEYS.map(async key => {
      try {
        const facility = await fetchFacility(key, date, signal);
        if (sequence !== loadSequence) return;
        state[key] = facility;
        delete errors[key];
      } catch (error) {
      ...
      renderState(state);
      const errorMessage = FACILITY_KEYS.map(item => errors[item]).filter(Boolean).join(" · ");
      const hasPendingFacility = FACILITY_KEYS.some(item => state[item] === null);
      if (errorMessage) setNotice(errorMessage, "error");
      else if (hasPendingFacility) setNotice("다른 시설의 예약 현황도 계속 확인하고 있어요.", "loading");
      else setNotice("");
    });
